        ...
        if os.path.isfile(path+"/combined.root"):
            ...
            for EvNr in FilteredEvIdxs[np.where(FilteredRuns==Run)[0][0]]:
                ...
                if TimeFormat=="LST":
                    ...
                    EventTime=np.append(EventTime,(UTC(TriggerTimes,EvIdx)-3)%24)
                else:
...
def GalacticNoiseSpectrum(StNr,ChNr,Run,EvtNr,Plot=False):
    ...
    import channelGalacticNoiseSpectrum
    ...
    Date=datetime.utcfromtimestamp(TriggerTimes[EvIdx]) - timedelta(hours=3, minutes=0)
